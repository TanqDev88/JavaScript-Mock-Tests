...
// //desde aca estos son los casos
// app.post('/clientes/:HashClient/altacliente', (req, res) => {
//     try {
//         L.debug('body', req.body);
//         L.info(req.params.HashClient);
//         const hc = req.params.HashClient;

//         const b = req.body;
//         if (!b) return res.status(400).json({ error: 'sin body' }); // TODO VER

//         if (utils.compareStrings(hc, '401')) return res.status(401).json({ error_code: 'Error de autenticación', message: ' mensaje' });
//         if (utils.compareStrings(hc, '412')) return res.status(412).json({ error_code: '101061', message: ' duplicado ' });
//         if (utils.compareStrings(hc, '103001')) return res.status(412).json({ error_code: '103001', message: ' error en la creación del cliente ' });
//         if (utils.compareStrings(hc, '103005')) return res.status(412).json({ error_code: '103005', message: ' error en la creación del cliente ' });
//         if (utils.compareStrings(hc, '101001')) return res.status(412).json({ error_code: '101001', message: ' error en la creación del cliente ' });
//         if (utils.compareStrings(hc, '107215')) return res.status(412).json({ error_code: '107215 ', message: ' inhabilitación de DNI ' });
//         if (utils.compareStrings(hc, '107526')) return res.status(412).json({ error_code: '107526', message: 'mail repetido' });
//         if (utils.compareStrings(hc, '701102')) return res.status(412).json({ error_code: '701102 ', message: ' error en sucursal ' });
//         if (utils.compareStrings(hc, '500')) return res.status(500).json({ error_code: 'service exception', message: ' error inesperado ' });

//         const resp = { ente: '55555' };
...
//     }
// });
// //Acas esta apertura de cuenta individuoo
// //simula el servicio de alta de cuenta
...
//     ) {
//         L.error('Solicitud incorrecta, falta algún parámetro obligatorio');
...
//     // Simulación de posibles errores basados en el ID del ente
//     if (utils.compareStrings(oficina, '401')) {
//         L.error('Error de autenticación');
...
//     if (utils.compareStrings(oficina, '258264')) {
//         L.error('Se superó la cantidad máxima de cuentas para el cliente');
...
//     if (utils.compareStrings(oficina, '101042')) {
//         L.error('No existe ente o cédula incorrecta');
//         return res.status(412).json({
//             error_code: '101042',
...
//     if (utils.compareStrings(oficina, '258235')) {
//         L.error('El oficial de cuenta no se encuentra para la sucursal o no está vigente');
//         return res.status(412).json({
//             error_code: '258235',
...
//     if (utils.compareStrings(oficina, '258266')) {
//         L.error('Error en la creacion Caja de Ahorro');
//         return res.status(412).json({
//             error_code: '258266',
...
//     if (utils.compareStrings(oficina, '500')) {
//         L.error('Error interno del servidor');
//         return res.status(500).json({
...
//     L.info('Apertura de cuenta en dólares exitosa');
//     return res.status(200).json({
...
// }
// //en pesos
//     L.info('Apertura de cuenta exitosa');
//     res.status(200).json({
//         cuenta_banco: '470309582121183',
//         cbu: '2850703540095821211838',
//         estado: 'A',
